...
export function canUseWebGL(): boolean {
  try {
    ...
    if (typeof console !== "undefined") {
      console.warn("[GenieMotion] WebGL not available", error);
    }
    return false;
  }
}
...
export function playGenie(
  ...
  if (!gl) {
    throw new Error("[GenieMotion] WebGL context unavailable");
  }
  ...
  if (isDevEnvironment && typeof console !== "undefined") {
    ...
    console.info("[GenieMotion] texture source", {
      tag: image.tagName,
      width: image.width,
      height: image.height,
  ...
  if (isDevEnvironment && typeof console !== "undefined") {
    console.info("[GenieMotion] WebGL playGenie", {
  ...
  const tick = (now: number) => {
    ...
    if (isDevEnvironment && typeof console !== "undefined") {
      ...
      if (error !== gl.NO_ERROR) {
        console.warn("[GenieMotion] WebGL error", error);
      }
    }
...
function createShaderProgram(
  ...
  if (!program) {
    throw new Error("[GenieMotion] Failed to create WebGL program");
  }
  ...
  if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
    ...
    throw new Error(`[GenieMotion] Failed to link program: ${info ?? "unknown"}`);
  }
...
function compileShader(gl: WebGLRenderingContext, type: number, source: string): WebGLShader {
  ...
  if (!shader) {
    throw new Error("[GenieMotion] Failed to create shader");
  }
  ...
  if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
    ...
    throw new Error(`[GenieMotion] Shader compile error: ${info ?? "unknown"}`);
  }
  return shader;
}
...
function createTexture(gl: WebGLRenderingContext, image: HTMLImageElement): WebGLTexture {
  ...
  if (!texture) {
    throw new Error("[GenieMotion] Failed to create texture");
  }
  ...
  try {
    ...
    if (isDevEnvironment && typeof console !== "undefined") {
      console.warn("[GenieMotion] texImage2D failed", {
        error,
