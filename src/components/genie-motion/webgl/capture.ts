...
export async function captureElementToImage(
  ...
  if (shouldDebugCapture()) {
    console.info("[GenieMotion] capture via html2canvas", {
      width,
      height,
      targetWidth,
      targetHeight,
...
async function captureWithHtml2Canvas(
  ...
  try {
    ...
    if (shouldDebugCapture()) {
      console.warn("[GenieMotion] html2canvas toDataURL failed", error);
    }
    throw error;
  }
  ...
  if (shouldDebugCapture()) {
    console.info("[GenieMotion] capture image ready", {
