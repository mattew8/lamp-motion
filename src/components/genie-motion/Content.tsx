...
export function GenieMotionContent({ children, glParams }: GenieMotionContentProps) {
  const { isOpen, origin, close } = useGenieMotionContext("Content");
  ...
  const ensureCanvas = useCallback(
    (rect: DOMRect, pixelRatio: number, offset: { x: number; y: number }) => {
      const overlay = overlayRef.current;
  ...
  const attemptPlayOpen = useCallback(
    async (node: HTMLElement, opId: number) => {
      ...
      if (!played && isDevEnvironment() && typeof console !== "undefined") {
        console.warn("[GenieMotion] WebGL open failed; Genie animation not played.");
      }
    },
    [playOpenWebGL],
  );
  ...
  const attemptPlayClose = useCallback(
    async (node: HTMLElement, opId: number) => {
      ...
      if (operationIdRef.current !== opId) return;
      if (!played && isDevEnvironment() && typeof console !== "undefined") {
        console.warn("[GenieMotion] WebGL close failed; Genie animation not played.");
      }
    },
    [playCloseWebGL],
  );
  ...
  if (!isValidElement(children)) {
    throw new Error("<GenieMotion.Content> expects a single React element child.");
  }
  ...
  return cloneElement(child, {
    ...
    "data-genie-motion-state": isOpen ? "open" : "closed",
    children: (
      <>
        <div ref={contentInnerRef} data-genie-motion-content style={contentWrapperStyle}>
          {originalChildren}
        </div>
        <div ref={overlayRef} aria-hidden="true" data-genie-motion-overlay style={overlayStyle} />
      </>
    ),
  });
}
